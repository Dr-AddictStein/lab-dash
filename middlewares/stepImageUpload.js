...
const storage = multerS3({
    ...
    key: function (req, file, cb) {
        ...
        const stepId = req.params.stepId; // Get labId from request parameters
        const fileName = `${Date.now().toString()}${path.extname(file.originalname)}`;
        const fullPath = `lab-content/${labId}/step-${stepId}/${fileName}`; // Construct the full path
        cb(null, fullPath);
    }
})
...
const uploadMiddleWareStepImage = upload;

export default uploadMiddleWareStepImage;